      ...
      {[...adj.keys()].map((key1) => (
        <Collapsible key={key1}>
          <div className="group relative w-full">
            <CollapsibleTrigger
              render={
                <DataCollapsibleButton className="absolute top-1/2 h-full -translate-y-1/2 hover:bg-transparent dark:hover:bg-transparent hover:[&_svg]:text-accent-foreground">
            ...
            {[...adj.get(key1)!.keys()].map((key2) => (
              <Collapsible key={key2}>
                <div className="group relative w-full">
                  <CollapsibleTrigger
                    render={
                      <DataCollapsibleButton className="absolute top-1/2 left-4 h-full -translate-y-1/2 hover:bg-transparent dark:hover:bg-transparent hover:[&_svg]:text-accent-foreground">
                        <ChevronRight className="size-3 text-muted-foreground" />
                      </DataCollapsibleButton>
                    }
                  ></CollapsibleTrigger>
